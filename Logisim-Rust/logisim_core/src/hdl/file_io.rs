...
impl HdlFile {
    ...
    pub fn open<P: AsRef<Path>>(
        ...
        let file = File::open(&file_path)
            .map_err(|e| HdlFileError::ReadError(format!("Failed to open file: {}", e)))?;
        ...
        for line_result in reader.lines() {
            let line = line_result
                .map_err(|e| HdlFileError::ReadError(format!("Failed to read line: {}", e)))?;
    ...
    pub fn save<P: AsRef<Path>>(file_path: P, editor: &dyn HdlContentEditor) -> HdlFileResult<()> {
        let file = OpenOptions::new()
            .write(true)
            .create(true)
            .truncate(true)
            .open(&file_path)
            .map_err(|e| HdlFileError::WriteError(format!("Failed to create file: {}", e)))?;
        ...
        writer
            ...
            .map_err(|e| HdlFileError::WriteError(format!("Failed to write data: {}", e)))?;

        writer
            .flush()
            .map_err(|e| HdlFileError::WriteError(format!("Failed to flush data: {}", e)))?;
    ...
    pub fn save_text<P: AsRef<Path>>(file_path: P, text: &str) -> HdlFileResult<()> {
        ...
        writer
            ...
            .map_err(|e| HdlFileError::WriteError(format!("Failed to write text: {}", e)))?;

        writer
            .flush()
            .map_err(|e| HdlFileError::WriteError(format!("Failed to flush text: {}", e)))?;
    ...
    pub fn is_vhdl_file<P: AsRef<Path>>(file_path: P) -> bool {
        matches!(
            Self::get_extension(file_path).as_deref(),
            Some("vhd") | Some("vhdl")
        )
    }
