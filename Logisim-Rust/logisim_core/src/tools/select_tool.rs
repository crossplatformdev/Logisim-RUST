...
impl SelectTool {
    ...
    fn find_components_in_rect(&self, canvas: &dyn Canvas, start: Location, end: Location) -> Vec<ComponentId> {
    ...
    fn apply_selection(&self, _canvas: &dyn Canvas, _components: Vec<ComponentId>, _mode: SelectionMode) {
...
mod tests {
    ...
    fn test_selection_mode_determination() {
        ...
        let shift_mods = KeyModifiers { shift: true, ..KeyModifiers::new() };
        assert_eq!(tool.get_selection_mode(&shift_mods), SelectionMode::Add);
        
        let ctrl_mods = KeyModifiers { ctrl: true, ..KeyModifiers::new() };
