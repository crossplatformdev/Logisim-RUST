...
impl AddTool {
    ...
    fn handle_place_component(&mut self, canvas: &dyn Canvas, event: &MouseEvent) -> ToolResult<()> {
...
impl Tool for AddTool {
    fn clone_tool(&self) -> Box<dyn Tool> {
        ...
        Box::new(AddTool {
            factory: Box::new(MockComponentFactory { name: self.name.clone() }), // Temporary hack
