...
fn test_signal_propagation() {
    ...
    let input_a = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "InputA".to_string());
    let input_b = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "InputB".to_string());
    let intermediate = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "Intermediate".to_string());
    let input_c = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "InputC".to_string());
    let output = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "Output".to_string());

    // Connect gate1: A & B -> Intermediate
    sim.netlist_mut()
        .connect(gate1_id, "A".to_string(), input_a)
        .unwrap();
    sim.netlist_mut()
        .connect(gate1_id, "B".to_string(), input_b)
        .unwrap();
    sim.netlist_mut()
        .connect(gate1_id, "Y".to_string(), intermediate)
        .unwrap();

    // Connect gate2: Intermediate & C -> Output
    sim.netlist_mut()
        .connect(gate2_id, "A".to_string(), intermediate)
        .unwrap();
    sim.netlist_mut()
        .connect(gate2_id, "B".to_string(), input_c)
        .unwrap();
    sim.netlist_mut()
        .connect(gate2_id, "Y".to_string(), output)
        .unwrap();
...
fn test_comprehensive_and_latch_circuit() {
    ...
    let and_output = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "AND_OUT".to_string());
    let clock = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "CLK".to_string());
    let latch_output = sim
        .netlist_mut()
        .create_named_node(BusWidth(1), "Q".to_string());

    // Wire up circuit: AND output -> Latch D input
    sim.netlist_mut()
        .connect(and_id, "A".to_string(), input_a)
        .unwrap();
    sim.netlist_mut()
        .connect(and_id, "B".to_string(), input_b)
        .unwrap();
    sim.netlist_mut()
        .connect(and_id, "Y".to_string(), and_output)
        .unwrap();

    sim.netlist_mut()
        .connect(latch_id, "D".to_string(), and_output)
        .unwrap();
    sim.netlist_mut()
        .connect(latch_id, "CLK".to_string(), clock)
        .unwrap();
    sim.netlist_mut()
        .connect(latch_id, "Q".to_string(), latch_output)
        .unwrap();
