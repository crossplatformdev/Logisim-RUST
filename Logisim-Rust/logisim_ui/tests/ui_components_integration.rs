...
fn test_properties_system_integration() {
    ...
    assert!(props.set_property("inputs", PropertyValue::Integer(4)).is_ok());
    assert!(props.set_property("inputs", PropertyValue::Integer(10)).is_err()); // Out of range
    
    assert!(props.set_property("facing", PropertyValue::Enum { 
        value: "North".to_string(), 
        options: vec!["East".to_string(), "West".to_string(), "North".to_string(), "South".to_string()]
    }).is_ok());
    
    assert!(props.set_property("negated", PropertyValue::Boolean(true)).is_ok());
    ...
    assert_eq!(new_props.get_property("inputs"), Some(&PropertyValue::Integer(3)));
}
