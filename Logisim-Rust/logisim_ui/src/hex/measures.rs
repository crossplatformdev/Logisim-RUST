...
impl Measures {
    ...
    pub fn to_address(&self, x: f32, y: f32, model: Option<&dyn HexModel>) -> Option<u64> {
        ...
        let base =
            self.get_base_address(Some(model)) + ((y / self.cell_height) as u64) * self.cols as u64;
        let mut offs =
            ((x - self.base_x) / (self.cell_width + (self.spacer_width + 2.0) / 4.0)) as i32;
    ...
    pub fn width_changed(&mut self, width: f32, model: Option<&dyn HexModel>) -> bool {
        ...
        let available_width = if self.guessed || self.cell_width <= 0.0 {
            ...
            let ret = ((width - self.header_width)
                / (self.cell_width + (self.spacer_width + 3.0) / 4.0))
                as usize;
            self.cols = if ret >= 16 {
                16
            } else if ret >= 8 {
        ...
        let line_width = self.header_width
            ...
            + ((self.cols / 4) as f32 - 1.0) * self.spacer_width;
    ...
    fn compute_cell_size(&mut self, model: Option<&dyn HexModel>) {
        ...
        self.preferred_width = self.header_width
            + self.cols as f32 * self.cell_width
            + (self.cols / 4) as f32 * self.spacer_width;
