...
#[cfg(feature = "gui")]
use egui::{
    Color32, Context, FontId, Key, Modifiers, Painter, Pos2, Rect, Response, Rounding, ScrollArea,
...
impl HexEditor {
    ...
    fn paint_content(&mut self, ui: &mut Ui, rect: Rect) {
        ...
        let visible_start = self
            .measures
            .to_address(0.0, rect.min.y, model_ref)
            .unwrap_or(0);
        let visible_end = self
            .measures
            .to_address(rect.width(), rect.max.y, model_ref)
            .unwrap_or(0);
        ...
        for row in start_row..=end_row {
            ...
            if self.show_addresses {
                let address_text = format!(
                    "{:0width$X}",
                    row_addr,
                    width = self.measures.get_label_chars()
                );
            ...
            for col in 0..cols {
                ...
                let hex_text =
                    format!("{:0width$X}", value, width = self.measures.get_cell_chars());
...
impl<'a> Widget for ScrollableHexEditor<'a> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        scroll_area.show(ui, |ui| self.editor.ui(ui)).inner
    }
}
