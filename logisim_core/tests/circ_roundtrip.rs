...
fn test_simple_and_gate_roundtrip() {
    ...
    for (original_comp, roundtrip_comp) in circuit.components.iter().zip(circuit2.components.iter())
    {
        ...
        assert_eq!(
            original_comp.attributes.len(),
            roundtrip_comp.attributes.len()
        );
    }
}
...
fn test_real_circ_file_roundtrip() {
    // Test with existing test resource file
    let test_file_path = "test_resources/MAINBOARD.circ";
    let sample_file_content = match std::fs::read_to_string(test_file_path) {
        Ok(content) => content,
        Err(_) => {
...
fn test_debug_parsing() {
    ...
    if let Some(circuit) = project.circuits.first() {
        ...
        for comp in &circuit.components {
            println!(
                "Component: {} at {:?} from lib {}",
                comp.name, comp.location, comp.lib
            );
...
fn test_debug_complex_parsing() {
    ...
    if let Some(circuit) = project.circuits.first() {
        ...
        for (i, comp) in circuit.components.iter().enumerate() {
            println!(
                "Component {}: {} at {:?} from lib {}",
                i, comp.name, comp.location, comp.lib
            );
            for (k, v) in &comp.attributes {
                println!("  {}: {}", k, v);
            }
