...
use logisim_ui::{EditHandler, LogisimApp, MainFrame, Selection};
...
fn test_circuit_file_discovery() {
    ...
    for dir in search_dirs {
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.extension().is_some_and(|ext| ext == "circ") {
                    found_files += 1;
    ...
    println!(
        "Circuit file discovery test completed - found {} files",
        found_files
    );

    // This test passes if we can run the search logic without panicking
    // The actual assertion would depend on the test environment
}
        ...
        if path.exists() {
            ...
            match CircIntegration::load_into_simulation(&path) {
                Ok(sim) => {
                    ...
                    println!(
                        "  Successfully loaded: {} nets, {} nodes",
                    ...
                    match result {
                        ...
                        Err(e) => {
                            println!(
                                "  UI integration failed (expected for unsupported components): {}",
                                e
                            );
