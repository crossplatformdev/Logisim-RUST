...
fn test_chronogram_basic_functionality() {
    ...
    let _node_id = simulation
        .netlist_mut()
        .create_named_node(BusWidth(1), "test_clk".to_string());

    // In headless mode, just verify simulation works
    let stats = simulation.stats();
    println!(
        "Simulation stats: events_processed={}, current_time={}",
        stats.events_processed,
        stats.current_time.as_u64()
    );
    ...
    assert!(!node_ids.is_empty(), "Should have at least one node");
}
...
fn test_chronogram_with_real_circuit() {
    ...
    #[cfg(feature = "gui")]
    {
        ...
        for i in 0..10 {
            ...
            if let Ok(has_more) = simulation.step() {
                ...
                println!(
                    ...
                    i,
                    simulation.current_time().as_u64()
                );
            }
        }
        ...
        println!(
            "Chronogram has {} signals",
            chronogram.model().signal_count()
        );
...
fn test_chronogram_signal_types() {
    ...
    println!(
        "Successfully tested chronogram with {} signals",
        model.signal_count()
    );
}
