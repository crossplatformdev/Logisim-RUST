...
impl Timeline {
    ...
    pub fn zoom_at_point(&mut self, zoom_delta: f32, point_x: f32) {
        ...
        if new_zoom != old_zoom {
            ...
            self.scroll_offset = Timestamp(self.scroll_offset.as_u64().saturating_sub(
                (offset_delta / self.zoom) as u64
            ));
        }
    }
        ...
        if ui.is_rect_visible(rect) {
            ...
            if let Some(cursor_time) = self.cursor_time {
                ...
                if cursor_x >= rect.left() && cursor_x <= rect.right() {
                    painter.line_segment(
                        [Pos2::new(cursor_x, rect.top()), Pos2::new(cursor_x, rect.bottom())],
