...
impl ChronogramPanel {
        ...
        ui.allocate_ui_at_rect(remaining_rect, |ui| {
            ui.horizontal(|ui| {
                ...
                let right_rect = Rect::from_min_size(
                    Pos2::new(
                        remaining_rect.min.x + left_width + 5.0,
                        remaining_rect.min.y,
                    ),
                    Vec2::new(
                        remaining_rect.width() - left_width - 5.0,
                        ...
                        for signal_info in self.model.signals() {
                            ...
                            if signal_rect.intersects(viewport) {
                                if let Some(signal_data) =
                                    self.model.get_signal_data(signal_info.id)
                                {
                                    let waveform = self
                                        .waveforms
    ...
    fn setup_default_signals(&mut self, _simulation: &Simulation) {
        ...
        let sysclk_info = SignalInfo::new(NodeId(0), "sysclk".to_string(), BusWidth(1), 0);
    ...
    fn record_simulation_state(&mut self, simulation: &Simulation, time: Timestamp) {
        ...
        if self.model.signal_count() == 1 && self.model.signals()[0].name == "sysclk" {
            ...
            self.model
                .record_signal_change(NodeId(0), time, clock_signal);
        }
    }
